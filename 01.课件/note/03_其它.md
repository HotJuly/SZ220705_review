...
- 2XX: 表示成功处理请求
  ...
  - 201->代表本次请求成功,但是参数不对
- 3XX: 需要重定向, 浏览器直接跳转
  - 302->代表地址重定向,比如说用户请求A链接,服务器返回响应的时候,引导浏览器跳转到B链接上
  - 304->代表协商缓存,比如说用户第二次请求某个资源,服务器发现浏览器上还存有该文件,那么服务器就不会返回这个文件,而且把响应码变为304,告知浏览器继续使用内存中的那个资源,可以减少请求消耗的时间,加快页面展示速度
- 4XX: 客户端请求错误
  - 401->代表身份验证失败,身份标识过期
  - 403->代表权限不够,拒绝返回,大概率是帐号权限不够,找后端就可以了
  - 404->代表找不到该资源,请求后端的接口或者文件,后端没有对应的资源,那么就会返回404
- 5XX: 服务器端错误
  - 500->代表服务器崩溃,无法处理该请求
  - 501->代表服务器无法处理本次请求,原因是请求方式不对
...
1. 配置通用的基础路径和超时时间
   ...
   2. 超时时间
      1. 小程序中请求最大并发数量是10个,Chorme和Firefox的请求最大并发数是6个
      2. 如果一个请求发送出去之后,在指定超时时间内没有返回,那么该请求就视为失败,该请求状态码会变为404
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
1. DNS 解析(将域名解析成对应的服务器IP)
   1. 问题:明明使用IP就可以访问服务器,为什么还要有域名
   2. 解答:域名的目的只有一个,就是方便记忆,方便推广
      1. 域名其实是对性能的劣化,他会导致用户访问网站速度更慢
...
- layout 布局(排版)：计算元素的位置和大小信息
  - 重排:当一个元素的大小位置发生变化,就会出现重排
- render 渲染：将颜色/文字/图片等渲染上去
  - 重绘:当一个元素的颜色,内容发生变化就会出现重绘,
  - 重排一定会出现重绘,重绘不一定有重排
